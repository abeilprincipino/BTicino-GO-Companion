...
func TestWebRTCService_OfferCandidateAndClose(t *testing.T) {
	...
	answer, err := service.Offer(source, "web-1", SessionDescription{Type: "offer", SDP: "offer-sdp"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if peer.remote != (SessionDescription{Type: "offer", SDP: "offer-sdp"}) || peer.local != peer.answer {
...
func TestWebRTCService_OfferRejectsDuplicateSession(t *testing.T) {
	...
	if _, err := service.Offer(source, "web-1", SessionDescription{Type: "offer", SDP: "offer-sdp"}); err != nil {
		t.Fatal(err)
	}
	if _, err := service.Offer(source, "web-1", SessionDescription{Type: "offer", SDP: "offer-sdp"}); !errors.Is(err, ErrSessionExists) {

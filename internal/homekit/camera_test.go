...
func TestCameraSession_SetupEndpointsUpdatesCharacteristicResponse(t *testing.T) {
	...
	if _, status := cameraService.SetupEndpoints.SetValueRequest(base64.StdEncoding.EncodeToString(setup), req); status != 0 {
...
func TestManager_ResourceHandlerServesDoorbellSnapshot(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/resource", bytes.NewBufferString(`{"aid":`+strconv.FormatUint(aid, 10)+`,"resource-type":"image"}`))
...
func TestManager_SyncPublishesOneDoorbellPressPerPhysicalRing(t *testing.T) {
	...
	doorbell.ProgrammableSwitchEvent.OnCValueUpdate(func(_ *characteristic.C, _, _ any, _ *http.Request) {
		presses++
	})

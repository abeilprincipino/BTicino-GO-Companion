...
func (s *Server) websocket(w http.ResponseWriter, r *http.Request) {
	...
	defer func() { s.clients.remove(client); conn.Close() }()
	if client.write(Message{Type: "state", Payload: mustJSON(s.snapshot())}) != nil {
		return
	}
	lastPing := time.Now()
	for {
...
func (s *Server) handleMessage(client *client, request *http.Request, message Message) {
	...
	case "command":
		if message.Action == "state.get" {
			_ = client.write(commandResult(message.ID, s.snapshot(), nil))
			return
		}
		if s.commands == nil {
...
func (s *Server) BroadcastState() {
	s.broadcast(Message{Type: "state", Payload: mustJSON(s.snapshot())})
}
